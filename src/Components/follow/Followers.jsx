...
export const Followers = () => {
  ...
  const getStudents = async (nextPage = 1) => {
    ...
    const request = await fetch(Global.url + 'follow/followers/'+studentId+'/'+nextPage, {
      method: 'GET',
      headers: {
    ...
    data.follows.forEach(follow =>{
        cleanStudent = [...cleanStudent, follow.student]
    });
  ...
  return (
    <>
      <header className="content__header">
        <h1 className="content__title">Seguidores de {studentProfile.name}</h1>
      </header>
