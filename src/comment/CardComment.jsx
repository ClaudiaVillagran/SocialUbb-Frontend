...
import { deleteMeComment, getComment } from "../features/publicationSlice";
import { Avatar } from "react-native-elements";

const CardComment = ({ comment, publicationId }) => {
    ...
    const deleteComment = (commentId) => {
        dispatch(deleteMeComment({ token, commentId, publicationId: publicationId }));
    };
    return (
            ...
            {comment ? (
                <View style={{ flexDirection: 'row', alignItems: 'center', width: '100%', justifyContent: 'space-between' }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', width: '100%' }}>
                        <View>
                                <Avatar size={32} rounded containerStyle={{ marginRight: 10, borderWidth: 1.5, borderColor: '#ffcc33' }} source={{ uri: `${comment.student.image}` }} />
                        </View>
                        <View>
